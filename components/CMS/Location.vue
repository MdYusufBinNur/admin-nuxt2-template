...
<!--      </template>-->
    ...
    >
      <v-card align="center">
        <v-card-text class="text-center pa-10 pp-body-reg-2">
          You are about to this district, do you want to proceed?
        </v-card-text>
        ...
        <v-container grid-list-sm class="pt-0">
          ...
          <v-row no-gutters class="pa-0">
            <v-col cols="12" class="pa-2">
              <label class="title">District Name</label>
              <v-text-field
                hide-details
                outlined
...
export default {
    ...
    locationList: {
      handler(nv, ov) {
        if (this.locationList && this.locationList.length) {
          this.items = JSON.parse(JSON.stringify(this.locationList))
        } else {
          this.initialize()
        }
      },
  ...
  methods: {
    ...
    changeStatus(item, title) {
      ...
      this.$nextTick(() => {
        this.$axios.post('/user-guide', this.editedItem)
          .then((response) => {
            if (response.data.error) {
              return this.$toast.error(response.data.message)
            }
            this.$toast.success(response.data.message)
            Object.assign(this.items[this.items.indexOf(item)], response.data.data)
          })
          .catch((error) => {
            console.log(error)
          })
    ...
    async save() {
      this.btnLoading = true
      await this.$axios.post('districts', this.editedItem)
        .then((res) => {
    ...
    async update() {
      ...
      await this.$axios.post('/districts/' + this.editedItem.id, formData)
        .then((res) => {
   ...
   async deleteSubs() {
      this.btnLoading = true
      await this.$axios.delete('/districts/' + this.editedItem.id)
        .then((res) => {
