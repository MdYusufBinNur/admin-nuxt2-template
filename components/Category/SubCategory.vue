...
<!--              <v-text-field v-model="editedItem.category_id" label="Category Name" outlined-->
<!--                            :rules="formRules.category_id"></v-text-field>-->
              ...
              <SelectCategory
                ...
                @setSelectedItem="getSelectedCategory"
              />
              <!-- Category Image -->
...
export default {
  name: 'SubCategory',
  components: {SelectCategory, PaginationClientSide},
  data() {
    return {
      editIcon,
      trashIcon,
  ...
  computed: {
    headers() {
      return [
        ...
        {text: this.$t('Category'), value: 'category.name', class: 'accentlight',},
    ...
    formRules() {
      ...
      return {
        name: [
          v => !!v || !this.editedItem.id || this.$t('Category name is required')
        ],
        // image: [
        ...
        category_id: [
          // Make the image field required only for new categories
          v => !this.editedItem.id || !!v || 'Category id is required'
        ],
      };
    },
  },
  ...
  methods: {
    ...
    makeDelete() {
      ...
      this.$axios.delete('sub-categories/' + this.editedItem.id)
        .then((response) => {
    ...
    getSelectedCategory(item) {
      this.editedItem.category_id = item
    },
    closeDialog() {
      this.dialog = false;
    ...
    saveCategory() {
      if (this.$refs.form.validate()) {
        ...
        this.$axios.post('sub-categories', formData)
          .then((response) => {
            this.$toast.success(response.data.message)
    ...
    editItem(item) {
      ...
      this.editedIndex = this.items.indexOf(item)
      this.$nextTick(() => {
        this.$refs.category.setId(item.id)
      })
      this.dialog = true
    },
    updateCategory() {
      ...
      this.editedItem.name && formData.append('name', this.editedItem.name)
      this.editedItem.category_id && formData.append('category_id', this.editedItem.category_id)
      this.editedItem.image && formData.append('image', this.editedItem.image)
      this.$axios.post('sub-categories/' + this.editedItem.id, formData)
        .then((response) => {
          this.$toast.success(response.data.message)
    ...
    getCategory() {
      this.loading = true
      this.$axios.get('sub-categories')
        .then((response) => {
          this.items = response.data.data.sub_categories
